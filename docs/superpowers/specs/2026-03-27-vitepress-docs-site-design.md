...
├── slides/                               ← unchanged PDFs (excluded from VitePress)
...
export default withMermaid(
  defineConfig({
    ...
    srcExclude: [
      ...
      'slides/**',                  // PDFs; docs/slides/index.md handles this section
...
- Push to `main` when paths match: `docs/**`, `exercises/**`, `*.md`, `index.md`, `solution/MyHANAApp/README.md`, `.github/workflows/deploy-docs.yml`
...
steps:
  ...
  - uses: actions/upload-pages-artifact@v3   # must be upload-pages-artifact, not upload-artifact
    if: github.event_name != 'pull_request'
    with:
...
| `docs/slides/index.md` | Slides section (fresh web content — no include) |
